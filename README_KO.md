...
| 확장 | 내용 |
|---|---|
| **한국 시장 적용** | Tushare → pykrx, ChatGLM → Gemini/Groq/GPT-4o-mini |
...
| 구성요소 | 비용 |
|---|---|
| Gemini, Groq, OpenDART, 네이버 API, pykrx | **모두 무료** |
...
[선택]  Groq/Llama    — 에이전트 sub-분석 (무료, 빠름, 지연 초기화).
[선택]  GPT-4o-mini   — 최종 합성기 + 임베딩만 사용. 품질·비용 최적점.
[선택]  google-genai  — Gemini 최신 SDK (google.generativeai deprecated 교체).
...
| LLM | ChatGLM2-6B + LoRA 파인튜닝 | **Gemini / Groq / GPT-4o-mini API 비교** |
...
│        ↓                                                         │
│  멀티 LLM: Gemini(무료) / Groq(무료) / GPT-4o-mini              │
│        ↓                                                         │
...
| 펀더멘털 에이전트 | **Groq/Llama** (무료) | RAG 보고서 문서 해석 |
| 감성 에이전트 | **Groq/Llama** (무료) | 뉴스 감성 분류 + Stage1 참조 |
...
감성 에이전트: 해당 종목의 최신 LLM 예측 조회
  예) "Gemini: 상승, Groq: 상승, GPT-4o-mini: 상승 (3/3 일치)"
                         ↓
RAG 뉴스 검색 결과 + Stage1 예측 → Groq에게 전달 → 감성 분류
```
...
│       │   ├── fundamental.py           # 펀더멘털 에이전트 (RAG + Groq)
│       │   ├── sentiment.py             # 감성 에이전트 (RAG + Stage1 + Groq)
...
| API | 발급처 | 비용 |
|---|---|---|
| Gemini | aistudio.google.com/app/apikey | 무료 |
| Groq | console.groq.com/keys | 무료 |
...
python stage1/llm_inference.py --models gemini groq openai
python stage1/postprocess.py
python stage1/backtest.py --weight 시총가중 --long_short 롱숏
```
...
|---|---|---|
| `pykrx` | KRX 주가·시총 | 무료 |
| `google-genai` | Gemini 최신 SDK — Stage 1 추론 | 무료 |
| `groq` | Llama 3.3 — Stage 1 + 에이전트 sub-분석 | 무료 |
